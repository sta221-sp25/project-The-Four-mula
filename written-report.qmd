...
imdb_data_cleaned %>% 
  mutate(popularity_mc = popularity - mean(popularity),
         log_budget_mc = log_budget_mc - mean(log_budget_mc))

m9 <- lm(log_revenue ~ vote_average + runtime + adult + log_budget_mc + popularity_mc + log_budget_mc * popularity_mc, data = imdb_data_cleaned)
tidy(m9) |>
  kable(digits = 3)
...
Model 2 outperforms Model 1 on both key metrics. Model 2 has a slightly lower RMSE, indicating better predictive accuracy, and a higher adjusted R-squared value, indicating a better fit and more variance explained by the model. Furthermore, since Model 2 has slightly lower AIC and BIC values, we select Model 2 as the better model. Looking at the VIF values for the predictors in Model 2, popularity and the interaction term between the log-transformed budget and popularity have extremely high VIF values, thus indicating a multicollinearity issue. However, the interaction term is statistically significant in the linear regression model and improves model performance. 

Thus, our final model is reflected by 

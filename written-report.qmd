...
```{r}
correlations <- imdb_data_cleaned |>
  select(vote_average, vote_count, runtime, log_budget, popularity) |>
  cor()
...
```{r}
max(imdb_data_cleaned$popularity)

ggplot(imdb_data_cleaned, aes(x = popularity)) +
  geom_histogram(fill= "violet", bins = 10) +
  labs(
    title = "Distribution of Popularity of Movies",
    x = "Popularity",
    y = "Count"
    )
```

From our exploration of the popularity data, we can see that its values range from 0-2994, with an extremely heavy right skew. So, we decided to bin it into 4 categories, "Low, "Medium", "High" and "Very High" with the following thresholds:
...
(maybe get rid of this, since we don't use this anywhere?)
...
After testing a linear regression model with each of the individual predictors to predict the log-transformed budget, since the p-value for each of these individual predictors is less than 0, we can reasonably conclude that there is a significant linear relationship between each of these individual predictors and the log-transformed revenue.
